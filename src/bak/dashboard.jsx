...
const Dashboard = () => {
    ...
    // Session storage se wallet address aur account number store karne ke liye
    ...
    // KYC STATUS - userData se 
    const kycStatus = userData?.kycstatus;

    // const baseUrl = "https://Cureness01.netlify.app/";
    ...
    // ✅ CURRENCY HELPERS - COMPONENT KE BAHR DEFINE KAREIN
    ...
    // ✅ FORMAT CURRENCY - DYNAMIC CONVERSION
    const formatCurrency = (amount) => {
        if (!amount && amount !== 0) return '$0.00';

        // Session se current currency lein
        const selectedCurrency = sessionStorage.getItem('selectedCurrency') || 'USD';
    ...
    // ✅ UPDATE CURRENCY ELEMENTS - DYNAMIC CONVERSION
    const updateCurrencyElements = () => {
        ...
        document.querySelectorAll(".currency1").forEach(function (el) {
            ...
            if (!isNaN(amount)) {
                const convertedAmount = amount * rate;
                el.innerHTML = symbol + Number(convertedAmount).toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                });
            }
    ...
    // ✅ Apply currency - SIRF INR (NO CONVERSION)
    useEffect(() => {
    ...
    // ✅ MODAL OPEN HONE PAR SESSION SE DATA LOAD
    useEffect(() => {
        if (showWithdrawModal) {
                                    ...
                                    <div className="animate__animated animate__fadeIn" style={{ marginTop: "-20px" }}>
                                        <div className="d-flex flex-wrap justify-content-between">
                                            ...
                                            {/* <div className='fundbtn'>
                                                <button type="button" title='fund-deposit' className="wallet-buttton b mt-2">
                                                    <MdAddCard size={20} />
                                                </button>
                                            </div> */}
                                            ...
                                            <Link to="/dashboard/Smartwallethistory">
                                                <p className="mb-1">
                                                    <strong title='Deposit History'>Smart Wallet : </strong>
                                        ...
                                        <div className='' style={{ marginTop: "-4px" }}>
                                            ...
                                            <div className='investment-wrapper  flex-wrap gap-1 gap-md-2'>
                            ...
                            <div className='meddle'>
                                <div className="methods-grid mt-3">
                                    ...
                                    <input
                                        ...
                                        value={selectedMethod === 'USDT TRC20' ? walletAddress : (selectedMethod === 'BANK CARD' ? accountNumber : '')}
                                        readOnly
                                ...
                                {/* ============================================
                        OTP SECTION - SEND & VERIFY SAME BUTTON
                        ============================================ */}
                                <div className='d-flex w-100' style={{ gap: '10px', alignItems: 'center' }}>
                                    <div className="input-container01 mt-3" style={{ flex: 1 }}>
                                    ...
                                    <button
                                        className="clear-btn"
                                        onClick={sendOtp}
                                        disabled={isOtpButtonDisabled() || !payoutStatus.active}
                                        style={{
                                            marginTop: '12px',
                                            height: '48px',
                                            padding: '0 20px',
                                ...
                                <button
                                    type="button"
                                    className={`modal-button mt-3 ${!payoutStatus.active || isWithdrawDisabled() ? 'withdraw-disabled' : ''}`}
                                    onClick={() => {
                                        if (!payoutStatus.active) {
                                            Swal.fire({
                                                icon: 'error',
                                                title: 'Payout Blocked!',
                                                text: payoutStatus.message || 'Payout is currently disabled. Please contact support.',
                                    ...
                                    disabled={!payoutStatus.active || isWithdrawDisabled()}
                                    style={{
                                        opacity: (!payoutStatus.active || isWithdrawDisabled()) ? 0.6 : 1,
                                        cursor: (!payoutStatus.active || isWithdrawDisabled()) ? 'not-allowed' : 'pointer',
                                        pointerEvents: (!payoutStatus.active || isWithdrawDisabled()) ? 'none' : 'auto',
                                    }}
                                >
                                ...
                                <div
                                    style={{
                                        color: payoutStatus.active ? '#198754' : '#dc3545',
                                        padding: '0px 15px',
                                        fontSize: '16px',
                                        fontWeight: '800',
                                        textAlign: 'center',
